...
进度条 + 重置时间；百分比保留一位小数，100 收敛整数、mini 窄档
退回整数），余额制面板（DeepSeek/NewAPI/Sub2API）显示余额。布局自适应：≤2 个分列排；3 个起切成 2 列
...
- Mirasim：桌面客户端的**本机接口**（127.0.0.1，无鉴权无 token）。
  ...
  同一窗口可能带档位子额度（如 `7d_fable`，预算约为总周窗的一半）：
  面板取**已用百分比最高**的那条显示——先撞墙的才是真实约束。
