...
显示口径两个开关彼此独立，四种组合都成立（`.env` 配置，或在菜单栏/
托盘里直接切，切完立刻重推一屏）：`INKSCRY_BAR_FILL` 管进度条方向
（缺省 `remaining` 满格代表额度充足，`used` 则条随消耗变长）、
`INKSCRY_PCT_MODE` 管大数字口径（缺省 `remaining` 剩余百分比，
`used` 已用百分比；菜单里会加「已用」前缀避免误读）。布局自适应：≤2 个分列排；3 个起切成 2 列
...
- **macOS 菜单栏程序（推荐）**：`pip install '.[menubar]'` 后运行
  ...
  CODEX 接口较慢随完整轮随后覆盖）、进度条方向（满格 = 额度充足 /
  满格 = 已用光）、数字口径（剩余 / 已用百分比；两者独立，切换即
  重推一屏）、屏幕管理（查看当前画面 / 清屏并
