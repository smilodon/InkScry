...
mini 窄档一律整数），余额制面板（DeepSeek/NewAPI/Sub2API）显示余额。
显示口径一个开关管到底（`INKSCRY_QUOTA_VIEW`，或在菜单栏/托盘里
直接切，切完立刻重推一屏）：缺省 `remaining` 看剩余量（满格 = 额度
充足），`used` 看已用量（数字是消耗量、条随消耗变长；菜单里会加
「已用」前缀避免误读）。**数字与条恒同向**——条紧贴数字、同宽、
无图例，含义全靠数字锚定，两者相反会被读成渲染出错。真要油表式
读法（数字看剩余、条看消耗）菜单里也有现成的一档，或手动用高级
覆盖 `INKSCRY_BAR_FILL` 单独指定条方向。布局自适应：≤2 个分列排；3 个起切成 2 列
...
- Mirasim：桌面客户端的**本机接口**（127.0.0.1，无需配任何凭据）。
  ...
  窗口的账号只有两档不展宽）。同一条 WS 上还推 Mirasim MITM 捕获的
  各 agent 用量：监听线程把 Codex 的 5h/7d 窗口一并写进 Codex 缓存
  （时间戳按捕获时刻），新鲜（<5 分钟）时同步轮直接复用、跳过
  chatgpt.com 的慢接口；闲置超时自动回退直查，且只写比现有缓存
  更新鲜的数据、绝不回滚。非官方接口（思路对齐
...
- **macOS 菜单栏程序（推荐）**：`pip install '.[menubar]'` 后运行
  ...
  （5/15/30/60 分钟运行期切换）、展开时刷新（点开菜单即补一轮同步，
  可选每次或至少间隔 1~15 分钟；走正常缓存/比对/防抖路径，频繁点开
  不会频繁闪屏；各供应商并发查询，快的一家 <1s 先上菜单，
  CODEX 接口较慢随完整轮随后覆盖）、显示口径（剩余量 / 已用量 /
  油表（数字看剩余、条看消耗），切换即重推一屏）、屏幕管理（查看当前画面 / 清屏并
